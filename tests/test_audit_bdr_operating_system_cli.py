...
MODULE_PATH = ROOT / "scripts" / "audit_bdr_operating_system.py"


def load_module():
    spec = importlib.util.spec_from_file_location("audit_bdr_operating_system_test", MODULE_PATH)
...
def test_run_audit_command_warns_and_writes_artifacts(tmp_path: Path, monkeypatch) -> None:
    ...
    fake_payload = {
        ...
        "rep_dashboard": "BDR Rep Queue",
        "manager_dashboard": "BDR Manager",
        "rep_widget_count": 20,
        "manager_widget_count": 24,
        "rep_chrome_ratio": 0.28,
        "manager_chrome_ratio": 0.31,
        "checks": [
            {
                "category": "story",
                "name": "sample_check",
