...
def test_build_release_packet_is_publish_ready_when_artifacts_are_clean(tmp_path: Path) -> None:
    ...
    fact_pack = _touch(director_run_dir / "jane" / "validated_bridge" / "validated-fact-pack.md")
    validation_report = _touch(director_run_dir / "jane" / "validated_bridge" / "validation-report.json")
    fill_payload = str(director_run_dir / "jane" / "validated_bridge" / "powerpoint-fill-payload.json")
    _write_json(
        Path(fill_payload),
        {
    ...
    deck_path = _touch(director_run_dir / "jane" / "deterministic_preview" / "deck.pptx")
    ...
    global_fact_pack = _touch(global_run_dir / "validated_bridge" / "validated-fact-pack.md")
    ...
    _write_json(
        global_run_dir / "manifest.json",
        {
            ...
            "powerpoint_fill_payload_path": str(global_run_dir / "validated_bridge" / "powerpoint-fill-payload.json"),
    ...
    _write_json(global_canonical_run_dir / "manifest.json", {"canonical_promotion": {"status": "ok"}})
    ...
    assert packet["director_release"]["targets"][0]["quarterly_pipeline_disclosure"] == {
        "director_name": "Jane Doe",
        "territory": "APAC",
    ...
    assert packet["global_release"]["quarterly_pipeline_disclosures"]["forward_quarter_fallbacks"] == [
        {
...
def test_build_release_packet_blocks_missing_validation_and_ambiguous_skips(tmp_path: Path) -> None:
    director_run_dir = tmp_path / "director-run"
    global_run_dir = tmp_path / "global-run"
            ...
            "targets": [
                {
                    ...
                    "stages": {
                        "validated_bridge": {
                            "validated_fact_pack": fact_pack,
                            "validation_report": str(director_run_dir / "missing-validation.json"),
                            "powerpoint_fill_payload": str(director_run_dir / "missing-fill.json"),
                        },
                        "deterministic_preview": {"deck_path": deck_path},
                        "deterministic_preview_render": {
                            "montage_path": montage_path,
    ...
    assert "Director PowerPoint review stage missing or ambiguous: Jane Doe." in packet["blockers"]
...
def test_write_release_bundle_updates_latest_aliases(tmp_path: Path) -> None:
    ...
    snapshot_latest = json.loads((output_root / "2026-04-10" / "latest.json").read_text())
