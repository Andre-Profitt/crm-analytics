...
import scripts.run_sales_global_canonical_shell_builder as module
...
def test_main_skip_mode_writes_authoring_bundle(monkeypatch, tmp_path: Path) -> None:
    ...
    argv = [
        "run_sales_global_canonical_shell_builder.py",
        "--snapshot-date",
        "2026-04-10",
    ...
    manifests = list((tmp_path / "runs" / "2026-04-10").glob("*/manifest.json"))
    assert manifests
...
def test_main_baseline_promotion_path_promotes_without_powerpoint(monkeypatch, tmp_path: Path) -> None:
    template = tmp_path / "SimCorp_PPT_Template.pptx"
    template.write_bytes(b"pptx")
    ...
    assert manifests
    manifest = json.loads(manifests[0].read_text())
    assert manifest["powerpoint_shell_authoring"]["reason"] == "baseline-deck promotion path"
