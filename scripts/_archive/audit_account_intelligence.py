#!/usr/bin/env python3
"""Audit Account Intelligence KPIs against account-intelligence rules."""
...
def _widget_titles(dashboard: dict[str, Any]) -> dict[str, str]:
    titles: dict[str, str] = {}
    for name, widget in dashboard["state"]["widgets"].items():
        ...
        if widget.get("type") == "text":
            content = params.get("content", {}).get("richTextContent", [])
            text = "".join(part.get("insert", "") for part in content).strip()
            label = text or label
        if label:
            titles[name] = html.unescape(label)
    return titles


def run_audit(live_export_dir: Path) -> dict[str, Any]:
    dashboard = _load(live_export_dir / "account_intelligence_kpis" / "dashboard.json")
    page_labels = _page_labels(dashboard)
    titles = _widget_titles(dashboard)
    ...
    checks.append(
        AuditCheck(
            category="persona",
            name="core_account_pages_present",
            passed=all(
                page in page_labels
                for page in [
                    "Data Quality",
    ...
    checks.append(
        AuditCheck(
            ...
            passed="Advanced Analytics" not in page_labels and "Statistical Analysis" not in page_labels,
            detail=f"Pages = {page_labels}",
        )
    )
    checks.append(
        AuditCheck(
            category="story",
            name="aum_surface_does_not_use_revenue_language",
            passed=not revenue_titles,
            detail=(
                "Revenue wording found in AUM-driven dashboard widgets: "
                + (json.dumps(revenue_titles, ensure_ascii=True) if revenue_titles else "none")
            ),
        )
    )
    checks.append(
        AuditCheck(
            category="story",
            name="aum_concentration_widget_explicit",
            passed=titles.get("p5_sec_concentration") == "Client Concentration by AuM"
            and titles.get("p5_ch_concentration") == "Top 20 Accounts by AuM",
            detail=(
                f"Section={titles.get('p5_sec_concentration', '')!r}, "
                f"Chart={titles.get('p5_ch_concentration', '')!r}"
            ),
        )
    )
    checks.append(
    ...
    checks.append(
        AuditCheck(
            category="action",
            name="cross_sell_queue_present",
            passed="p4_tbl_ws_opps" in dashboard["state"]["widgets"],
            detail="Cross-sell page includes the whitespace opportunity queue table.",
        )
    )
    checks.append(
        AuditCheck(
            category="visual",
            name="segment_mix_present",
            passed=all(viz in viz_counts for viz in ["donut", "stackhbar", "comparisontable"]),
            detail=f"Visualization types present = {sorted(viz_counts)}",
        )
    )
