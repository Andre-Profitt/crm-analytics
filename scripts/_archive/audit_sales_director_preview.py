#!/usr/bin/env python3
"""Audit a populated Sales Director preview deck for shell leakage and title quality."""
...
DEFAULT_SHELL_CONTRACT = Path(__file__).resolve().parents[1] / "config" / "sales_director_monthly_shell.json"
...
def analyze_preview(
    *,
    ...
    director_name = str(fill_payload.get("director_name") or "").strip()
    territory = str(fill_payload.get("territory") or "").strip()
    ...
    for index, slide_def in enumerate(body_slides, start=3):
        ...
        support_level = payload_slide.get("support_level") or (slide_def.get("data_contract") or {}).get("support_level")
        title = first_meaningful_text(texts)

        if slide_id != "appendix-notes" and support_level != "placeholder" and title == slide_def["title"]:
            findings.append(
                {
                    "severity": "warn",
        ...
        if display_reason:
            disclosure = {
                ...
                "quarterly_pipeline_title": quarter_title or None,
                "quarterly_pipeline_footnote": footnote or None,
            }
            quarterly_pipeline_disclosures.append(disclosure)
            if display_reason == "forward_quarter_fallback" and not (footnote and footnote in joined):
                findings.append(
                    {
                        "severity": "error",
                        ...
                        "slide_id": slide_id,
                        "message": (
                            f"{director_name or 'Director book'} is using a forward-quarter fallback but the "
                            "required footnote is not visible on the rendered slide."
                        ),
                    }
                )
