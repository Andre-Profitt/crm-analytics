...
DEFAULT_SHELL_CONTRACT = (
    Path(__file__).resolve().parents[1] / "config" / "sales_director_monthly_shell.json"
)
...
def analyze_preview(
    *,
    ...
    for index, slide_def in enumerate(body_slides, start=3):
        ...
        support_level = payload_slide.get("support_level") or (
            slide_def.get("data_contract") or {}
        ).get("support_level")
        title = first_meaningful_text(texts)

        if (
            slide_id != "appendix-notes"
            and support_level != "placeholder"
            and title == slide_def["title"]
        ):
            findings.append(
                {
                    "severity": "warn",
        ...
        if display_reason:
            ...
            if display_reason == "forward_quarter_fallback" and not (
                footnote and footnote in joined
            ):
                findings.append(
                    {
                        "severity": "error",
...
def audit_preview(
    ...
    shell_contract_path: Path = DEFAULT_SHELL_CONTRACT,
) -> dict[str, Any]:
