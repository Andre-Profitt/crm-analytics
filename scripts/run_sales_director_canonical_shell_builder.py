#!/usr/bin/env python3
"""Author and promote canonical director shells from the SimCorp PowerPoint template."""
...
from build_sales_director_shell_author_prompt import build_prompt, load_json
...
DEFAULT_RUN_ROOT = REPO_ROOT / "output" / "sales_director_canonical_shell_builder"
...
def canonical_shell_name(director_name: str, territory: str) -> str:
    return f"Sales Director Monthly Shell - {director_name} ({territory}).pptx"
...
def promote_canonical_shell(
    *,
    ...
    filename = canonical_shell_name(director_name, territory)
    stable_path = canonical_root / filename
    dated_path = canonical_root / snapshot_date / filename
...
def main() -> None:
    ...
    run_dir = (
        ...
        / args.snapshot_date
        / timestamp_slug()
        / args.director_name.lower().replace(" ", "-")
    )
    ...
    working_deck_path = editable_dir / canonical_shell_name(args.director_name, args.territory)
    ...
    if args.baseline_deck_path and args.powerpoint_mode == "build":
        raise ValueError("Use either --baseline-deck-path for direct promotion or --powerpoint-mode build for live shell authoring, not both.")
