...
| --- | ---------------------------- | ---------------- | -------------------- | -------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `dq_missing_decision_reason` | SF Report        | `00OTb000008el0PMAQ` | exists - clean (Phase 2.8) | yes                  | Audit row 26: "Missing Decision Reason", `THIS_FISCAL_YEAR` filter. Must switch to calendar year per hard rule 1. Also flagged: field not in detail columns (field name TBD - see spec open question 3).                                                                                                   |
...
| 3   | `dq_missing_won_loss_cfq`    | SF Report        | `00OTb000008SqblMAC` | exists - clean (Phase 2.8) | yes                  | Audit row 30: "Won Loss Info Missing CFQ", `THIS_FISCAL_QUARTER` filter. Must switch to calendar quarter per hard rule 1. CFQ field name still needs verification (see spec open question 7).                                                                                                              |
| 4   | `dq_missing_amount`          | SF Report        | `00OTb000008TZqcMAG` | exists - clean (Phase 2.8) | yes                  | Audit row 25: "Missing Amount on Open Opps", `THIS_FISCAL_YEAR` filter. Must switch to calendar year per hard rule 1. Spec note: should also flag `APTS_Opportunity_ARR__c=null` per hard rule 2 (ARR is canonical).                                                                                       |
...
**UPDATED 2026-04-08 (Phase 2.6, audit commit `d4d8b63`):** All 5 widgets BUILT via `POST /analytics/reports` and added as new components on Dashboard 2 (now 18 components total). Each report shows the count of NON-COMPLIANT opportunities (Option A from the Phase 2.6 brainstorm - count-only, not ratio). Reports live in folder `005QA000003DUwWYAW`. **Important:** The Phase 1 audit's stem matcher does NOT recognize these new widgets as matching the `pc_*` spec entries because the new component titles (e.g., "P2.6 Mid-Stage Opps Lacking NextStep") don't share stems with the spec KPI bullet ("Process compliance rates"). The matcher tags them ORPHAN, but they ARE the canonical sources for the spec widgets - this row's `Source ID` field is the source of truth for Phase 4 deck rebuild, not the audit's `matched_spec_id` field.

| #   | Spec widget                        | Canonical source | Source ID            | Verification status            | Phase 2.5 build needed | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --- | ---------------------------------- | ---------------- | -------------------- | ------------------------------ | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 6   | `pc_next_step_documented`          | SF Report        | `00OTb000008fAjZMAU` | built (Phase 2.6) - simplified | no                     | Built 2026-04-08 as "P2.6 Mid-Stage Opps Lacking NextStep". **Simplification:** the `Opportunity.NextStep IS NULL` filter was DROPPED because the field is not accessible to the running user. The report now shows ALL mid-stage open opps as a superset (overcounts). Live count: 191. Sales Ops can drill down. Per-region grouping by `Opportunity.Sales_Region__c`.                                                                                                                |
| 7   | `pc_land_commercial_approval_flow` | SF Report        | `00OTb000008fAlBMAU` | built (Phase 2.6) - clean      | no                     | Built 2026-04-08 as "P2.6 Land Deals Lacking Commercial Approval Flow". Filter: `Type='Land' AND IsClosed=false AND StageName IN (3-6) AND Stage_20_Approval__c=false AND Submit_for_Stage_20_Review__c=false`. Live count: 5. Per-region grouping. Per Sales Handbook V4 slide 12 commercial approval gate.                                                                                                                                                                            |
| 8   | `pc_recent_activity_logged`        | SF Report        | `00OTb000008fAmnMAE` | built (Phase 2.6) - simplified | no                     | Built 2026-04-08 as "P2.6 Active Opps With No Activity Logged Ever". **Simplification:** the OR clause `LastActivityDate < TODAY-30 OR LastActivityDate IS NULL` was simplified to just `LAST_ACTIVITY equals ""` (IS NULL only). Captures the most-broken subset (opps with no activity ever) but misses opps that had activity 31+ days ago. Live count: 276. Per-region grouping.                                                                                                    |
| 9   | `pc_won_loss_reason_documented`    | SF Report        | `00OTb000008fAoPMAU` | built (Phase 2.6) - clean      | no                     | Built 2026-04-08 as "P2.6 Closed Opps This Quarter Lacking Reason". Filter: `IsClosed=true AND CloseDate IN THIS_QUARTER AND Reason_Won_Lost__c IS NULL`. Note: uses bare `THIS_QUARTER` calendar token (per Phase 1.5 finding that `THIS_CALENDAR_QUARTER` is invalid in this org). Live count: 9. Per-region grouping. Distinct from widget 3's report.                                                                                                                               |
| 10  | `pc_stage_age_within_threshold`    | SF Report        | `00OTb000008fArdMAE` | built (Phase 2.6) - simplified | no                     | Built 2026-04-08 as "P2.6 Mid-Stage Opps Exceeding 60-Day Age". **Simplification:** uses a single 60-day threshold across stages 3-6 (not the per-stage thresholds the spec proposed of Stage 3=60d, 4=45d, 5=30d, 6=15d) AND uses `standardDateFilter CUSTOM` with absolute `endDate=today-60` (the relative `LAST_UPDATE lessThan "TODAY-60"` form is rejected by reportFilters in this org). Live count: 17. Per-region grouping. Per-stage thresholds deferred to a v2 enhancement. |
...
| 19  | `ph_stale_opportunities`           | SF Report        | `00OTb000008TZgvMAG` | exists - clean (Phase 2.8)                          | yes                                 | Audit row 28: "Stale Opportunities", `THIS_FISCAL_YEAR` filter. Must switch to calendar year per hard rule 1. List view sibling of widget 10 (`pc_stage_age_within_threshold`). Current value: 1.3K rows.                                                                                                                                                                                                   |
| 20  | `ph_no_activity_30_plus`           | SF Report        | `00OTb000008TaEnMAK` | exists - clean (Phase 2.8) + ARR aggregation defect | yes                                 | Audit row 27: "No Activity 30+ Days - Open Opps", `THIS_FISCAL_YEAR` filter. TWO defects: (1) fiscal-to-calendar fix required per hard rule 1; (2) aggregation uses `s!AMOUNT` (standard Amount) instead of `APTS_Opportunity_ARR__c` per hard rule 2. Current value: 1214.99M (Amount). Both defects must be fixed before deck rebuild.                                                                    |
