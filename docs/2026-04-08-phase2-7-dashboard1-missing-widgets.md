...
> **2026-04-10 update — forecast subregion bug fix:** The APAC region filter in this doc previously grouped `Middle East & Africa` with `APAC`. That is wrong for CRO forecast tie-out; Middle East & Africa rolls up under EMEA in the live Salesforce forecast tree. The APAC and EMEA widget filters below were updated accordingly. Single source of truth for territory rollups: [`config/territory_mappings.json`](/Users/test/crm-analytics/config/territory_mappings.json). Full context: [`docs/2026-04-10-forecast-subregion-bug-handoff.md`](/Users/test/crm-analytics/docs/2026-04-10-forecast-subregion-bug-handoff.md).
...
| 2   | `pipeline_overview_emea`       | `IsClosed=false AND CloseDate IN THIS_QUARTER AND Sales_Region__c IN ('United Kingdom & Ireland','Central Europe','Northern Europe','Southwestern Europe','Middle East & Africa')` | `STAGE_NAME`                       | `s!Opportunity.APTS_Opportunity_ARR__c.CONVERT`           | stacked bar by stage                                                |
...
| 4   | `pipeline_overview_apac`       | `IsClosed=false AND CloseDate IN THIS_QUARTER AND Sales_Region__c='APAC'`                                                                                                          | `STAGE_NAME`                       | `s!Opportunity.APTS_Opportunity_ARR__c.CONVERT`           | stacked bar by stage                                                |
...
| 7   | `land_stage3_no_approval_apac` | `TYPE='Land' AND STAGE_NAME='3 - Engagement' AND Opportunity.Stage_20_Approval__c='False' AND Sales_Region__c='APAC'`                                                              | (table)                            | `RowCount, s!Opportunity.APTS_Opportunity_ARR__c.CONVERT` | tabular list                                                        |
